...
## <div id='1.2'/>1.2. 범위
PaaS-TA AP min은 bosh-deployment를 기반으로 한 BOSH 환경에서 설치하며 paasta-deployment v5.6.4-min의 설치를 기준으로 가이드를 작성하였다.  
PaaS-TA AP min은 VMware vSphere, Google Cloud Platform, Amazon Web Services EC2, OpenStack, Microsoft Azure 등의 IaaS를 지원하며,  paasta-deployment v5.6.4-min에서 검증한 IaaS 환경은 AWS, OpenStack 환경이다.
...
$ cd ~/workspace
$ git clone https://github.com/PaaS-TA/paasta-deployment.git -b v5.6.4-min paasta-deployment-min
```
...
paasta-deployment v5.6.4은 Ubuntu bionic stemcell 1.34을 기반으로 한다.  
